...
RSpec.describe "Importu Converters" do
  subject(:context) { Importu::ImportContext.new({}, fields: {}, converters: converters) }
  let(:converters) { definition.config[:converters] }
  let(:definition) do
    Class.new do
  ...
  describe ":raw converter" do
    let(:context) do
      fields = {
        field1: { label: "field1", required: true },
        field3: { label: "field3", required: true },
      }

      data = { "field1" => " value1 ", "field2" => "value2" }
      Importu::ImportContext.new(data, fields: fields, converters: converters)
    end

    it "uses definition's label as key when looking up data" do
      expect(context.convert(:field1, :raw)).to eq " value1 "
    end

    it "raises an exception if field definition is not defined" do
      expect { context.convert(:field2, :raw) }.to raise_error(Importu::InvalidDefinition)
    end

    it "raises MissingField if field data not defined" do
      expect { context.convert(:field3, :raw) }.to raise_error(Importu::MissingField)
    end
  end
  ...
  describe ":clean converter" do
    it "returns nil if raw data is nil" do
      ...
      expect(context.convert(:field1, :clean)).to be nil
    end
    ...
    it "returns stripped value if a string" do
      ...
      expect(context.convert(:field1, :clean)).to eq "abc  123"
    end

    it "returns original value if not a string" do
      raw_val = Time.now
      definition.converter(:raw) { raw_val }
      expect(context.convert(:field1, :clean)).to eq raw_val
    end
  end
  ...
  describe ":decimal converter" do
    it "returns nil if clean value is nil" do
      definition.converter(:clean) { nil }
      expect(context.convert(:field1, :decimal)).to be nil
    end

    it "returns decimal if clean value returns an decimal" do
      clean_val = BigDecimal("92.25")
      definition.converter(:clean) { clean_val }
      expect(context.convert(:field1, :decimal)).to eq clean_val
    end

    it "converts clean value to decimal if not an decimal type" do
      definition.converter(:clean) { "29.4" }
      expect(context.convert(:field1, :decimal)).to eq BigDecimal("29.4")
    end

    it "converts whole values to decimal" do
      definition.converter(:clean) { "77" }
      expect(context.convert(:field1, :decimal)).to eq BigDecimal("77.0")
    end

    it "raises an exception if clean value is not a valid decimal" do
      definition.converter(:clean) { "4d6point3" }
      expect { context.convert(:field1, :decimal) }.to raise_error(Importu::FieldParseError)
    end
  end
  ...
  describe ":boolean converter" do
    it "returns nil if clean value is nil" do
      definition.converter(:clean) { nil }
      expect(context.convert(:field1, :boolean)).to be nil
    end

    it "returns true if clean value is true" do
      definition.converter(:clean) { true }
      expect(context.convert(:field1, :boolean)).to eq true
    end
    ...
    it "returns false if clean value is false" do
      ...
      expect(context.convert(:field1, :boolean)).to eq false
    end
    ...
    it "raises an exception if the value is invalid" do
      ...
      expect { context.convert(:field1, :boolean) }.to raise_error(Importu::FieldParseError)
    end
  ...
  describe ":date converter" do
    it "returns nil if clean value is nil" do
      definition.converter(:clean) { nil }
      expect(context.convert(:field1, :date)).to be nil
    end
    ...
    context "when a format is not specified" do
      it "tries to guess the date format (DD/MM/YYYY is default?)" do
        ...
        expect(context.convert(:field1, :date)).to eq expected
      end

      it "raises an exception if the date is invaild" do
        definition.converter(:clean) { "2012-04-32" }
        expect { context.convert(:field1, :date) }.to raise_error(Importu::FieldParseError)
      end
    end
    ...
    context "when a format is specified" do
      it "parses dates that match the format" do
        ...
        expect(context.convert(:field1, :date, format: "%m/%d/%Y")).to eq expected
      end
  ...
  describe ":datetime converter" do
    it "returns nil if clean value is nil" do
      definition.converter(:clean) { nil }
      expect(context.convert(:field1, :datetime)).to be nil
    end
    ...
    context "when a format is not specified" do
      it "tries to guess the date format (DD/MM/YYYY is default?)" do
        ...
        expect(context.convert(:field1, :datetime)).to eq expected
      end

      it "raises an exception if the datetime is invaild" do
        definition.converter(:clean) { "2012-04-32 15:41:22" }
        expect { context.convert(:field1, :datetime) }.to raise_error(Importu::FieldParseError)
      end
    end
    ...
    context "when a format is specified" do
      it "parses datetimes that match the format" do
        ...
        expect(context.convert(:field1, :datetime, format: "%m/%d/%Y %H:%M")).to eq expected
      end
